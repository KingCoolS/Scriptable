...
/**
 * Clean files
 * https://raw.githubusercontent.com/Honye/scriptable-scripts/master/dist/Clean%20Files%202.js
 *
 * @version 2.2.0
 * @author Honye
 */
  ...
  * {
  ...
  const view = async (data) => {
    ...
    if (Number(isDirectory)) {
      let unit = i18n(['items', '项']);
      let list = fm.listContents(filePath).sort().map((name) => {
...
presentList({
  ...
  list: [
    {
      name: i18n(['Local cache directory', '缓存目录']),
      filePath: FileManager.local().cacheDirectory(),
      isDirectory: true
    },
    {
      name: i18n(['Local temporary directory', '临时目录']),
      filePath: FileManager.local().temporaryDirectory(),
      isDirectory: true
    },
    {
      name: i18n(['Local documents directory', '文档目录']),
      filePath: FileManager.local().documentsDirectory(),
      isDirectory: true
    },
    {
      name: i18n(['Local library directory', '库目录']),
      filePath: FileManager.local().libraryDirectory(),
      isDirectory: true
    },
    // {
    //   name: i18n(['iCloud document directory', 'iCloud 文档']),
    //   filePath: FileManager.iCloud().documentsDirectory(),
    //   isDirectory: true
    // },
    // {
    //   name: i18n(['iCloud library directory', 'iCloud 库']),
